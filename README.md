...
2. Pick **MULTIPLAYER** or **ZOMBIES** on the selector. Under Multiplayer, choose **MOD MENUS** (replaces `maps/mp/gametypes/_clientids.gsc`) or **GAME MODES** (replaces `maps/mp/_development_dvars.gsc`, where custom game-mode scripts live); both run in the same `codmp.elf` process, so switching between them keeps your attachment. Zombies attaches to `codzm.elf`; while attached to one executable the other tab refuses to switch until you Detach, and Detach (or Disconnect) refuses while scripts are still injected until you Uninject All. The exact slot is shown under the selector.
3. In the tool: **CONNECT** → **ATTACH** → pick a menu → **INJECT**. Once connected the button reads **Disconnect**, and once attached it reads **Detach**; press them to drop the session or forget the game process. **Library** opens the picker: choose one of the bundled packs for the current mode, or press **CUSTOM GSC** to use your own PS4 `.gscc` file (*royal_menu_ps4.gscc* is the known-good one). The library window filters by name as you type and shows each pack's conversion notes; packs with several scripts are injected together and rolled back together if one fails. See [`library/README.md`](library/README.md).
