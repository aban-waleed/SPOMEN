...
public sealed class Ps4DebugClient : IDisposable
{
	...
	{
		PS4DBG pS4DBG = dbg ?? throw new Exception("Not connected");
		try
		{
			return pS4DBG.Version;
		}
		catch
		{
			return pS4DBG.GetConsoleDebugVersion();
		}
	}
	...
	public List<(int Pid, string Name)> ListProcesses()
	{
		...
		ProcessList processList = obj.GetProcessList();
		if (typeof(ProcessList).GetField("processes", BindingFlags.Instance | BindingFlags.NonPublic)?.GetValue(processList) is Process[] array)
		{
			Process[] array2 = array;
			foreach (Process process in array2)
			{
				list.Add((process.pid, process.name));
			}
		}
		return list;
	}
