...
/// <summary>Mirrors src/BO2InjectorGUI/GiveWindow.xaml.cs.</summary>
public partial class GiveWindow : Window
{
	...
	private readonly Ps4DebugClient dbg = null!;
	...
	public GiveWindow(Ps4DebugClient dbg, int pid)
	{
		...
		Opened += async delegate { if (dbg != null) { await RefreshRoster(); refresh.Start(); } };
	...
	private async Task RefreshRoster(bool quiet = false)
	{
		...
		try
		{
			Dictionary<int, bool> keep = rows.ToDictionary(r => r.Slot, r => r.Selected);
			List<InjectorEngine.ClientInfo> roster = await Task.Run(() => Engine().ReadRoster(pid));
			rows = roster.Select(c => new Row
			{
	...
	private async Task Apply()
	{
		...
		List<int> slots = rows.Where(r => r.Selected).Select(r => r.Slot).ToList();
